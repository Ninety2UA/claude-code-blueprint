...
If no plan file found, report: "No plan file found. Write a plan first with the brainstorming skill or specify a path."
...
**If called from a pipeline** (build-pipeline or ship-pipeline): skip execution options and return to the calling workflow. The pipeline controls execution in its next stage.
...
**2. Parallel Orchestration (orchestrate skill)** — Executes the wave plan: independent tasks run in parallel within each wave. Faster total time for plans with concurrent tasks.

**3. Agent Teams (team-execution skill)** — Collaborative teammates with file ownership and shared task list. Best for 4+ tasks touching different areas. Requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`.
